...
def binaryToDecimal(binary):
    ...
    while (binary != 0):
        ...
        binary = binary // 10
        i += 1
    box = mb.showinfo("Abara Ka Dabara", f"Your Number Is : {decimal}")
    if box == "ok":
        exit()
...
titletxt = Label(frameTop, text="Magic Box",
                 fg="deep pink", font="lucida 25 bold")
titletxt.pack(fill=BOTH)

titletxt = Label(
    frameTop, text="Think About A Number In\nRange 0 To 60", font="lucida 12 bold")
titletxt.pack(fill=BOTH)

frameMiddle = Frame(root)
frameMiddle.pack(fill=BOTH)

shownums = Text(frameMiddle, width=20, height=7, font="lucida 16 bold",
                fg="white", bg="grey10", relief=SUNKEN, bd=10)
shownums.pack(fill=X, padx=3, pady=5)

frameBottom = Frame(root)
frameBottom.pack(fill=BOTH)

Label(frameBottom, text="Is Your Number On The Blackboard?", fg="grey1",
